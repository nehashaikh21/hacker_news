...
function Article () {
    ...
    useEffect(() => {
        ...
        const fetchNews = () =>{
            console.log(query)
        fetch(`http://hn.algolia.com/api/v1/search?query=${query}&page=${currentPage}`)
        .then((res) => res.json())
        .then((json) => {
            console.log(json)
            setNews(json.hits);
    ...
    return (
        <div>
            ...
            <br/><br/>
              {isLoading?<ReactPlaceholder type='media' rows={8} ready={false} showLoadingAnimation={true} color={"#6596fe"}>
                </ReactPlaceholder>: <div>{news?.map((news) => {
                return (
                    <section>
